...
6. Verify `schema_migrations` contains migrations through `011_async_pvp_harbours.sql`. Startup rejects any database missing `011` (it backs asynchronous PvP), so apply it before deploying the PvP release.
