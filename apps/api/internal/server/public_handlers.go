...
func (s *Server) shareHTML(w http.ResponseWriter, r *http.Request) {
	...
	title := fmt.Sprintf("@%s cleared their GitHub history on GitHarbour", u.Login)
	description := "Archived pre-launch GitHarbour result."
	if g.Ruleset == "contribution_targets_v2" && len(g.Shots) > 0 {
		description = fmt.Sprintf("%d contribution days · %d shots · %.0f%% accuracy · %s to %s", g.TargetCount, len(g.Shots), 100*float64(g.TargetCount)/float64(len(g.Shots)), g.PeriodStart, dateEnd(g.PeriodStart))
	}
